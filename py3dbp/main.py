...
class Bin:
    ...
    def __init__(self, partno, WHD, max_weight,corner=0,put_type=1):
        ...
        self.gravity = []
        self.cumulative_volume = 0.0  # 新增累计体积属性

    def add_item(self, item):
        """ 添加物品并更新累计体积 """
        if self.checkFit(item):
            self.items.append(item)
            self.cumulative_volume += item.getVolume()
            return True
        return False
...
class Packer:
    ...
    def get_total_volume(self, bin):
        """ 获取单个箱子的累计体积 """
        return bin.cumulative_volume

    def get_all_volumes(self):
        """ 获取所有箱子的体积字典 """
        packVolume = packVolume + Pack
        return { packVolume }
        ...
        for idx,bin in enumerate(self.bins):
            ...
            # Deviation Of Cargo Gravity Center 
                try:
                    ...
                    # 可以选择记录日志或忽略
                    print(f"Error calculating gravity for bin {idx}: {e}")
                    continue
...
class ContainerVisualizer:
    def __init__(self, container, highlight_bins=None,fig=None, ax=None):
        """
        :param container: Container实例
        :param highlight_bins: 需要高亮的bin集合（partno列表或集合）
        """
        self.container = container
        self.highlight_bins = set(highlight_bins) if highlight_bins else set()
    ...
    def plot(self):
        """绘制完整的集装箱视图"""
        self._plot_container()    # 绘制集装箱外框
        self._plot_bins()         # 绘制所有箱子
        self._set_axes()          # 配置坐标轴
        plt.tight_layout()

    def _plot_bins(self):
        """动态判断并设置颜色"""
        for bin in self.container.bins:
            global_pos = (
                self.container.global_position[0] + bin.position[0],
                self.container.global_position[1] + bin.position[1],
                self.container.global_position[2] + bin.position[2]
            )
            
            # 动态判断颜色
            if bin.partno in self.highlight_bins:
                color = (1, 0, 0, 0.6)  # 红色
            else:
                color = (0.5, 0.5, 0.5, 0.6)  # 默认灰色
                
            self._plot_cuboid(global_pos, bin.WHD, color=color, edgecolor='gray')

    def _plot_container(self):
        """绘制集装箱外框"""
        pos = self.container.global_position
        dims = self.container.template['dims']
        self._plot_cuboid(pos, dims, color=(0.2,0.2,0.2,0.1), edgecolor='black')

 

    def _plot_cuboid(self, pos, dims, color, edgecolor='gray', label=None):
        """通用长方体绘制函数"""
        x, y, z = pos
        l, w, h = dims
    ...
    def _add_labels(self, pos, dims, partno):
        """添加箱体标签"""
        x, y, z = pos
        l, w, h = dims
        # 在箱体中心位置添加文字标签
    ...
    def _set_axes(self):
        ...
        gp = container.global_position
        dims = container.template['dims']
        
        self.ax.set_xlim(gp[0], gp[0] + dims[0])
        self.ax.set_ylim(gp[1], gp[1] + dims[1])
        self.ax.set_zlim(gp[2], gp[2] + dims[2])
...
class Painter:
    ...
    def plotBoxAndItems(self, title="", alpha=0.2, write_num=False, fontsize=10, fig=None, ax=None):
        if not fig:
            fig = plt.figure()
        if not ax:
        ...
        # 原绘图代码改为使用传入的ax
        self._plotCube(ax, 0, 0, 0, float(self.width), float(self.height), float(self.depth), color='black', mode=1, linewidth=2, text="")
        ...
        for item in self.items:
            ...
            if item.typeof == 'cube':
                 # plot item of cube
                self._plotCube(ax, float(x), float(y), float(z), float(w),float(h),float(d),color=color,mode=2,text=text,fontsize=fontsize,alpha=alpha)
            elif item.typeof == 'cylinder':
                # plot item of cylinder
                self._plotCylinder(ax, float(x), float(y), float(z), float(w),float(h),float(d),color=color,mode=2,text=text,fontsize=fontsize,alpha=alpha)
            
            counter = counter + 1  

        
        plt.title(title)
        self.setAxesEqual(ax)
        return fig, ax
...
class BinDraw:
    def __init__(self, partno, WHD, max_weight, position, corner, put_type, info=None):
        self.partno = partno
        self.WHD = WHD
        self.max_weight = max_weight
        ...
        self.corner = corner
        self.put_type = put_type
        self.info = info or {}  # 存储自定义信息

class Container:
    def __init__(self, container_id, template_name, global_position=(0,0,0)):
        self.id = container_id
        self.template = ContainerTemplate.get_template(template_name)
        self.global_position = global_position
        self.bins = self._init_bins()
        
    def _init_bins(self):
        return [
            BinDraw(
                partno=bin_config['id'],
                WHD=bin_config['dims'],
                max_weight=bin_config.get('max_weight', 1000),
                position=bin_config['position'],
                corner=0,
                put_type=1
            ) for bin_config in self.template['bins']
        ]
    def get_painter_data(self):
        return {
...
class ContainerTemplate:
    ...
    def _load_templates(cls) -> Dict[str, Any]:
        """加载JSON配置文件"""
        config_path = Path(__file__).parent.parent / "configs/container_templates.json"
    ...
    def get_template(cls, name: str) -> Dict[str, Any]:
        ...
        if cls._templates is None:
            cls._templates = cls._load_templates()
            
        template = cls._templates.get(name)
        if not template:
            raise ValueError(f"未找到集装箱模板：{name}")
            
        # 添加数据验证
        required_keys = {"description", "dims", "bins"}
        if not required_keys.issubset(template.keys()):
            missing = required_keys - template.keys()
            raise ValueError(f"模板 '{name}' 缺少必要字段：{missing}")
